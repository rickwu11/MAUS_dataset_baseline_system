...
def RMSE_align(reference, target, roi):
    ROI = slice(int(roi[0]), int(roi[1]), 1)

    align_ppi = target[ROI]
    num_of_shift = len(reference)-len(align_ppi)
    error = []
    for i in range(num_of_shift):
        error.append(RMSE(align_ppi,reference[i:i+len(align_ppi)]))

    # Find the Minimum point
    index_min = error.index(min(error))

    return index_min-int(roi[0])

def RMSE(rri,ppi):
    # Check the sequence is in same length
    assert(len(rri)==len(ppi))
